...
public class CityCommandItem extends RecyclerView.Adapter<CityCommandItem.ViewHolder> {
    ...
    public CityCommandItem(List<City_Command_item> city_command_item_List){                     //构造函数
        mCityCommandItem = city_command_item_List;
    }

    @Override
    public CityCommandItem.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {     //ViewHolder的OnCreate函数
        if(mContext == null){
            mContext = parent.getContext();
        }
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_card_command,parent,false);
        final CityCommandItem.ViewHolder holder = new CityCommandItem.ViewHolder(view);
...
//                Toast.makeText(v.getContext(),"进入ing",Toast.LENGTH_SHORT).show();
                ...
                City_Command_item chapter = mCityCommandItem.get(position);
...
//                mContext.startActivity(intent);
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        holder.author_message.setText(chapter.getAuthor_message());
